...
from component.PageFrameCommponent import InferenceTaskPage1, InferenceTaskPage2, \
    InferenceTaskPage3, InferenceTaskPage4, InferenceContinueTaskPage3
...
class InferenceMainContainPage(Frame,Observer):
    ...
    def updates(self, arg):
        ...
        if flag == 'start_new':
            ...
            self.contain = InferenceContinueTaskContainPage(self.frame,self.width,self.height)
...
class InferenceContinueTaskContainPage(Frame, Observable):
    def __init__(self, parent, width, height):
        Frame.__init__(self, parent)
        Observable.__init__(self)
        self.pack()
        ...
        page = InferenceContinueTaskPage3(self.frame_middle, self.width, self.height * 3 / 4)
        self.continue_before_list_management['page3']=page
        self.continue_before_list_management['page3'].pack()
        ...
        for i in range(1,4):
            self.radio_value_list[f'page{i}'].set(1)
        self.radio_label_text[f'page{self.current_status}'].set("当前")
        self.radio_label_text[f'page{self.current_status+1}'].set("下一步")

    def change_frame(self, flag):
        if flag == 'pre':
            self.continue_before_list_management[f'page{self.current_status}'].pack_forget()
            self.continue_before_list_management[f'page{self.current_status - 1}'].pack()
            self.change_radio_status('pre')
            self.change_radio_label_text('pre')
            self.current_status -= 1
            ...
            if ready:
                ...
                if self.exist(f"page{self.current_status + 1}"):
                    ...
                    self.continue_before_list_management[f'page{self.current_status + 1}'].pack()
                    if self.current_status+1 == 4:
                        self.continue_before_list_management[f'page{self.current_status }'].logRuntimeJson()
                else:
                    (page_key, page) = self.create_page(self.current_status + 1)
                    self.continue_before_list_management[page_key] = page
                    if self.current_status+1 == 4:
                        self.continue_before_list_management[f'page{self.current_status }'].logRuntimeJson()
                self.current_status += 1
                #messagebox.showinfo('提示', message)
    ...
    def exist(self,page_key):
        if page_key in self.continue_before_list_management:
            return True
        else:
            return False
    def create_page(self,page):
        if page==3:
            new_page = InferenceContinueTaskPage3(self.frame_middle, self.width, self.height * 3 / 4)
        elif page==4:
            new_page = InferenceTaskPage4(self.frame_middle, self.width, self.height * 3 / 4)
        page_key=f'page{page}'
        return (page_key,new_page)
    ...
    def check_current_status_ready(self):
        if self.current_status == 3:
            self.continue_before_list_management[f'page{self.current_status}'].output_path_load()
            ready=self.continue_before_list_management[f'page{self.current_status}'].get_task_output_ready()
            if ready:
                return (True,"已暂停任务")
            else:
                return (False,"输出位置不能为空")
